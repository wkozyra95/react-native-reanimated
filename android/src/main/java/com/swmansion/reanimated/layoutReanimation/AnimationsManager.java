...
public class AnimationsManager {
  private static final String[] LAYOUT_KEYS = {
    Snapshooter.originX, Snapshooter.originY, Snapshooter.width, Snapshooter.height
  };
    ...
    ArrayList<String> keys =
        new ArrayList<String>(
            Arrays.asList(
                ...
                Snapshooter.originX,
                Snapshooter.originY,
                Snapshooter.globalOriginX,
                Snapshooter.globalOriginY));
    for (String key : keys) {
  ...
  public void setNewProps(
      ...
      Integer parentTag) {
    float x =
        (props.get(Snapshooter.originX) != null)
            ? ((Double) props.get(Snapshooter.originX)).floatValue()
            : PixelUtil.toDIPFromPixel(view.getLeft());
    float y =
        (props.get(Snapshooter.originY) != null)
            ? ((Double) props.get(Snapshooter.originY)).floatValue()
            : PixelUtil.toDIPFromPixel(view.getTop());
    float width =
        (props.get(Snapshooter.width) != null)
            ? ((Double) props.get(Snapshooter.width)).floatValue()
            : PixelUtil.toDIPFromPixel(view.getWidth());
    float height =
        (props.get(Snapshooter.height) != null)
            ? ((Double) props.get(Snapshooter.height)).floatValue()
  ...
  public void notifyAboutSnapshots(Snapshooter before, Snapshooter after) {
    ...
    for (View view : allViews) {
      ...
      if (view instanceof AnimatedRoot) {
        ArrayList<View> pathToTheRoot =
            (ArrayList<View>)
                before.capturedValues.get(view.getId()).get(Snapshooter.pathToTheRootView);
  ...
  boolean dfs(View view, boolean disappearingAbove) {
    ...
    if (view instanceof ViewGroup) {
      ...
      for (int i = 0; i < vg.getChildCount(); ++i) {
        ...
        boolean childAns =
            dfs(child, disappearing || disappearingAbove); // we still want to go down
        active = active || childAns;
      }
    }

    if ((!disappearingAbove) && (state == ViewState.ToRemove) && (!active)) {
      ViewTraverser.internalTraverse(
          view,
          (View curView) -> {
