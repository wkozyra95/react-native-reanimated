...
public class ReactBatchObserver {
          ...
          public void run() {
            try {
              ...
              ShadowNodeRegistry shadowNodeRegistry =
                  (ShadowNodeRegistry) shadowRegistry.get(mUIImplementation);
  ...
  public void willMount() {
    ...
    BiFunction<NativeViewHierarchyManager, BiFunction<AnimatedRoot, Integer>>
        goThroughAffectedWithLambda =
            (hierarchyManager, lambda) -> {
    ...
    mUIManager.prependUIBlock(
        nativeViewHierarchyManager -> {
          BiFunction<AnimatedRoot, Integer> lambda =
              (AnimatedRoot root, Integer tag) -> {
                Snapshooter snapshooter = new Snapshooter(tag);
                if (root != null) {
                  ViewTraverser.traverse(
                      root,
                      (view) -> {
